<template>
    <div class="kb" v-if="showKb">
        ...
        <div class="kb-keyboard">
            ...
            <div class="kb-keyboard__panle row-wrap">
                <!--省份简写键盘-->
                ...
                <!--数字字母键盘-->
                <div v-if="isNum" v-for="(numItem, j) in '1234567890' " :key="j" @tap="tapKeyboard" :data-index="j" :data-val="numItem" class="kb-keyboard__td-num kb-keyboard__td-theme row-center" hover-class="kb-keyboard__td-tap-theme" hover-start-time="0" hover-stay-time="40">{{numItem}}</div>
                <div v-if="!isNum" v-for="(numItem, q) in '1234567890' " :key="q" class="kb-keyboard__td-num kb-keyboard__td-tap-theme row-center">{{numItem}}</div>
                <!-- <div v-if="isNum" v-for="(numItem, j) in '1234567890' " :key="j" @tap="tapKeyboard" :data-index="j" :data-val="numItem" class="kb-keyboard__td-num kb-keyboard__td-theme row-center" hover-class="kb-keyboard__td-tap-theme" hover-start-time="0" hover-stay-time="40">{{numItem}}</div> -->
                <!-- <div v-if="!isNum" v-for="(numItem, q) in '1234567890' " :key="q" class="kb-keyboard__td-num kb-keyboard__td-tap-theme row-center">{{numItem}}</div> -->
                <!--字母键盘-->
                <div v-for="(alItem , k) in 'QWERTYUPASDFGHJKLZX巛CVBNM' " :key="k" @tap="tapKeyboard" :data-index="k" :data-val="alItem" class="kb-keyboard__td-num kb-keyboard__td-theme row-center" hover-class="kb-keyboard__td-tap-theme" hover-start-time="0" hover-stay-time="40">
                    <div :data-index="k" :data-val="alItem" v-if="alItem === '巛'" class="kb-keyboard__del"></div>
                    <div :data-index="k" :data-val="alItem" v-if="alItem !== '巛' && k !== '巛'">
                        {{alItem}}
                    </div>
                </div>

                <!-- 完成按钮 -->
                <div v-if="textArr.length > 5" @tap="tapFinished" class="kb-keyboard__td kb-keyboard__finished row-center" hover-class="kb-keyboard__td-tap-fin-theme" hover-start-time="0" hover-stay-time="60">{{extraKey}}</div>
                <div v-if="textArr.length < 6" class="kb-keyboard__td kb-keyboard__finished-base row-center">{{extraKey}}</div>
            </div>
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        tapKeyboard(e) {
            ...
            if (this.tapVal === '巛') {
                ...
                if (
                    this.textArr.length === 6 &&
                    this.tapVal !== ''
                ) {
                    this.tapVal = '';
                }
            } else {
                if (this.textArr.length > 6) return false;
                if (this.textArr.length > 5) {
                    this.textBaseArr.splice(5, 1, this.tapVal);
                    this.textArr.splice(5, 1, this.tapVal);
        ...
        // energy(index) {
        //     if (index === 6 && this.textArr.length === 6) {
        //         this.isEnergy = false;
        //     }
        // }
    },
