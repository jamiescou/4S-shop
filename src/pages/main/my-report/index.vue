<template>
  <div class="report_car_box">
    ...
    <div class="baoyang_content_box">
      ...
      <div class="baoyang_list" v-for="(item, index) in listReport" :key="index" @click="checkReportItem(index)">
        <div class="baoyang_item">
          {{item.name}}
        </div>
        <div class="_right">
          <div>{{item.error}}项 <img class="error_icon" src="../../../../static/image/error.png" /></div>
          <div>{{item.nomal}}项 <img class="nomal_icon" src="../../../../static/image/nomal.png" /></div>
...
export default {
  ...
  methods: {
    checkReportItem (index) {
      wx.navigateTo({
        url: '../report-item/main?id=' + index
      })
    },
    checkRecordError () {
      wx.navigateTo({
        url: '../report-error/main'
      })
    }
  },
  created () {
